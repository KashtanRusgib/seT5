/**
 * @file posix.h
 * @brief POSIX → seT5 Syscall Translation Layer
 *
 * Maps standard POSIX system calls to the seT5 ternary kernel ABI.
 * This allows unmodified POSIX applications to run on seT5 by
 * translating binary syscall conventions to balanced ternary.
 *
 * Design:
 *   - POSIX return codes (int) → trit validity (T/U/F)
 *   - POSIX file descriptors → seT5 capability slots
 *   - POSIX signals → seT5 notifications
 *   - POSIX mmap → seT5 page allocation with ternary validity
 *
 * The translation layer is stateless per call — it does not maintain
 * its own state beyond what the underlying seT5 kernel provides.
 *
...
/* ---- POSIX ↔ seT5 Mapping Table --------------------------------------- */

/**
 * POSIX errno → ternary validity mapping.
 *
 * POSIX errors are negative integers; seT5 uses trit:
 *   success (≥ 0)  → TRIT_TRUE
...
/**
 * seT5 trit result → POSIX return code.
 */
...
/**
 * Map POSIX fd → seT5 capability slot.
 *
 * POSIX: fd 0=stdin, 1=stdout, 2=stderr, 3+ = user fds
 * seT5:  cap slots 0-2 reserved, 3+ user caps
 *
...
/* ---- POSIX Signal ↔ seT5 Notification --------------------------------- */

/**
 * Map POSIX signals to seT5 notification bits.
 *
 * POSIX: SIGTERM=15, SIGKILL=9, SIGUSR1=10, SIGUSR2=12, ...
 * seT5:  notification bits 0-31 in ipc_notification_t
 *
...
/* ---- POSIX mmap ↔ seT5 Memory Allocation ------------------------------ */

/**
 * Translate POSIX mmap flags to seT5 page allocation.
 *
 * POSIX mmap: addr, length, prot, flags, fd, offset
 * seT5:       mem_alloc(state, owner) → page index
 *
 * Key differences:
 *   - seT5 pages are Unknown-initialised (not zero-filled like MAP_ANONYMOUS)
 *   - seT5 uses ternary validity instead of PROT_READ/WRITE/EXEC
 *   - seT5 scrubs on free (security guarantee)
 */
...
/**
 * Convert POSIX prot flags to nearest seT5 capability permissions.
 * seT5 caps have: read, write, grant
 * PROT_EXEC maps to a separate "execute" capability (not yet in seT5 ABI)
 */
...
/* ---- POSIX Process ↔ seT5 Thread -------------------------------------- */

/**
 * Map POSIX process/thread operations to seT5 scheduler.
 *
 ...
 *
 * seT5 threads have ternary priority levels, not POSIX nice values.
 */
 ...
 *
 * Takes a POSIX syscall number and translates to seT5 kernel call.

...
static inline int dev_trit_ioctl(dev_trit_state_t *dev, int cmd, void *arg)
{
    ...
    case TRIT_IOCTL_GET_REG:
    {
        ...
        if (!r || r->reg_index < 0 || r->reg_index >= MR_NUM_REGS)
            return -22;
    ...
    case TRIT_IOCTL_DOT:
    {
        ...
        if (!d)
            return -22;
        if (d->reg_a < 0 || d->reg_a >= MR_NUM_REGS)
            return -22; /* VULN-fix: OOB reg_a */
        if (d->reg_b < 0 || d->reg_b >= MR_NUM_REGS)
            return -22; /* VULN-fix: OOB reg_b */
    ...
    case TRIT_IOCTL_FFT:
    {
        ...
        if (!f)
            return -22;
        if (f->reg_index < 0 || f->reg_index >= MR_NUM_REGS)
            return -22; /* VULN-fix: OOB reg */
        if (f->offset < 0 || f->offset >= 32)
            return -22; /* VULN-fix: OOB offset */
    ...
    case TRIT_IOCTL_RADIX_CONV:
    {
        ...
        if (!r)
            return -22;
        if (r->reg_index < 0 || r->reg_index >= MR_NUM_REGS)
            return -22; /* VULN-fix: OOB reg */

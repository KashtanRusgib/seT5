/**
 * @file sel4_ternary.h
 * @brief seT5 Moonshot — Full seL4 Kernel Object Model in Balanced Ternary
 *
 ...
 *
 * Key seT5 innovations over binary seL4:
...
#ifndef SET5_SEL4_TERNARY_H
#define SET5_SEL4_TERNARY_H
...
#define SET5_MAX_CNODE_SLOTS    256   /* CNode capacity (radix bits = 8) */
#define SET5_MAX_TCBS            64   /* Max threads */
#define SET5_MAX_ENDPOINTS       32   /* Max IPC endpoints */
#define SET5_MAX_NOTIFICATIONS   32   /* Max notification objects */
#define SET5_MAX_UNTYPED         64   /* Max untyped memory regions */
#define SET5_MAX_FRAMES         256   /* Max physical frames */
#define SET5_MAX_PAGE_TABLES     16   /* Max page tables */
#define SET5_MAX_IRQS            32   /* Max hardware interrupts */
#define SET5_MAX_ASID_POOLS       4   /* Max ASID pools */
#define SET5_MAX_DOMAINS          3   /* Ternary: {F, U, T} domains */
 ...
 *
 * Every operation in seT5 is mediated by capabilities.
...
static inline set5_cap_t cap_mk_null(void) {
    set5_cap_t c = {0};
    c.type     = KOBJ_NONE;
...
static inline set5_cap_t cap_mk(kobj_type_t type, int idx,
 ...
 */
static inline set5_cap_t cap_derive(const set5_cap_t *parent,
                                    trit rd, trit wr, trit gr, int badge) {
    set5_cap_t child = *parent;
 ...
 */
static inline void cap_revoke(set5_cap_t *cap) {
    if (!cap) return;
 ...
 */
static inline trit cap_has_right(const set5_cap_t *cap, trit right) {
 ...
 *
 * seT5 innovation: each slot has a ternary validity field that
 * propagates through lookups — an Unknown slot means "pending
 * capability transfer, not yet committed."
 */
typedef struct {
    set5_cap_t  slots[SET5_MAX_CNODE_SLOTS];
...
static inline void cnode_init(set5_cnode_t *cn) {
    if (!cn) return;
    for (int i = 0; i < SET5_MAX_CNODE_SLOTS; i++)
 ...
 *   - Guard mismatch
 */
static inline set5_cap_t *cnode_lookup(set5_cnode_t *cn, int index) {
    if (!cn || cn->validity == TRIT_FALSE) return NULL;
    if (index < 0 || index >= SET5_MAX_CNODE_SLOTS) return NULL;
    return &cn->slots[index];
}

static inline int cnode_insert(set5_cnode_t *cn, int index, set5_cap_t cap) {
    if (!cn || cn->validity == TRIT_FALSE) return -1;
    if (index < 0 || index >= SET5_MAX_CNODE_SLOTS) return -1;
...
static inline int cnode_delete(set5_cnode_t *cn, int index) {
    if (!cn || cn->validity == TRIT_FALSE) return -1;
    if (index < 0 || index >= SET5_MAX_CNODE_SLOTS) return -1;
 ...
 */
static inline void cnode_revoke_all(set5_cnode_t *cn) {
    if (!cn) return;
    for (int i = 0; i < SET5_MAX_CNODE_SLOTS; i++) {
...
static inline void tcb_init(set5_tcb_t *tcb, int id) {
    if (!tcb) return;
    tcb->id            = id;
...
static inline void tcb_configure(set5_tcb_t *tcb, int cspace, int vspace,
...
static inline void tcb_set_priority(set5_tcb_t *tcb, trit prio) {
    if (!tcb || tcb->validity == TRIT_FALSE) return;
    tcb->priority = prio;
}

static inline void tcb_resume(set5_tcb_t *tcb) {
    if (!tcb || tcb->validity == TRIT_FALSE) return;
    tcb->state      = THREAD_RUNNING;
    tcb->timeslice  = tcb->max_timeslice;
}

static inline void tcb_suspend(set5_tcb_t *tcb) {
    if (!tcb) return;
    tcb->state = THREAD_BLOCKED;
}

static inline void tcb_destroy(set5_tcb_t *tcb) {
    if (!tcb) return;
    tcb->state    = THREAD_INACTIVE;
...
typedef struct {
    ...
    int               send_queue[SET5_MAX_TCBS];
    int               send_head, send_tail;

    int               recv_queue[SET5_MAX_TCBS];
    int               recv_head, recv_tail;

    /* Last transferred message (for debugging/proof) */
    set5_ipc_msg_t    last_msg;
} set5_endpoint_t;

static inline void endpoint_init(set5_endpoint_t *ep, int id) {
    if (!ep) return;
    ep->id          = id;
 ...
 */
static inline trit endpoint_send_enqueue(set5_endpoint_t *ep, int thread_id) {
    if (!ep || ep->validity == TRIT_FALSE) return TRIT_FALSE;
    int next = (ep->send_tail + 1) % SET5_MAX_TCBS;
...
static inline int endpoint_send_dequeue(set5_endpoint_t *ep) {
    if (!ep || ep->send_head == ep->send_tail) return -1;
    int tid = ep->send_queue[ep->send_head];
    ep->send_head = (ep->send_head + 1) % SET5_MAX_TCBS;
    if (ep->send_head == ep->send_tail) ep->queue_state = EP_IDLE;
    return tid;
}

static inline trit endpoint_recv_enqueue(set5_endpoint_t *ep, int thread_id) {
    if (!ep || ep->validity == TRIT_FALSE) return TRIT_FALSE;
    int next = (ep->recv_tail + 1) % SET5_MAX_TCBS;
...
static inline int endpoint_recv_dequeue(set5_endpoint_t *ep) {
    if (!ep || ep->recv_head == ep->recv_tail) return -1;
    int tid = ep->recv_queue[ep->recv_head];
    ep->recv_head = (ep->recv_head + 1) % SET5_MAX_TCBS;
 ...
 */
static inline trit endpoint_transfer(set5_endpoint_t *ep,
                                     const set5_ipc_msg_t *msg) {
...
typedef struct {
    ...
    int              wait_queue[SET5_MAX_TCBS];
    int              wait_head, wait_tail;
} set5_notification_t;

static inline void notification_init(set5_notification_t *ntf, int id) {
    if (!ntf) return;
    ntf->id        = id;
 ...
 */
static inline trit notification_signal(set5_notification_t *ntf, int bit) {
 ...
 */
static inline int notification_poll(const set5_notification_t *ntf) {
 ...
 */
static inline int notification_wait(set5_notification_t *ntf) {
    if (!ntf || ntf->validity == TRIT_FALSE) return -1;
    int consumed = 0;
    for (int i = 0; i < 32; i++) {
        if (trit2_reg32_get(&ntf->signal_word, i) == TRIT2_TRUE) {
 ...
 *
 * seT5 innovation: Untyped memory is Unknown-initialised (not zero).
...
static inline void untyped_init(set5_untyped_t *ut, int id,
 ...
 */
static inline int untyped_retype(set5_untyped_t *ut, kobj_type_t type,
                                 int obj_size) {
 ...
 */
static inline void untyped_revoke(set5_untyped_t *ut) {
    if (!ut) return;
    ut->watermark = 0;
    ut->children_count = 0;
...
static inline void frame_init(set5_frame_t *fr, int id, int phys) {
    if (!fr) return;
    fr->id           = id;
...
static inline trit frame_map(set5_frame_t *fr, int vaddr, int asid) {
...
static inline trit frame_unmap(set5_frame_t *fr) {
    if (!fr || fr->validity == TRIT_FALSE) return TRIT_FALSE;
    fr->mapped_vaddr = -1;
    fr->mapped_asid  = -1;
    return TRIT_TRUE;
...
#define SET5_PTE_PER_TABLE  256 /* 256 entries per table */

typedef struct {
    int         id;
    trit        validity;
    int         level;          /* Page table level (0=root, 1=leaf) */
    set5_pte_t  entries[SET5_PTE_PER_TABLE];
    int         mapped_count;   /* Number of valid mappings */
} set5_page_table_t;

static inline void page_table_init(set5_page_table_t *pt, int id, int level) {
    if (!pt) return;
    pt->id           = id;
    pt->validity     = TRIT_TRUE;
    pt->level        = level;
    pt->mapped_count = 0;
    for (int i = 0; i < SET5_PTE_PER_TABLE; i++) {
 ...
 */
static inline trit page_table_map(set5_page_table_t *pt, int index,
                                  int frame_idx, trit wr, trit ex, int asid) {
    if (!pt || pt->validity == TRIT_FALSE) return TRIT_FALSE;
    if (index < 0 || index >= SET5_PTE_PER_TABLE) return TRIT_FALSE;
...
static inline trit page_table_unmap(set5_page_table_t *pt, int index) {
    if (!pt || pt->validity == TRIT_FALSE) return TRIT_FALSE;
    if (index < 0 || index >= SET5_PTE_PER_TABLE) return TRIT_FALSE;
 ...
 */
static inline int page_table_lookup(const set5_page_table_t *pt, int index) {
    if (!pt || pt->validity == TRIT_FALSE) return -1;
    if (index < 0 || index >= SET5_PTE_PER_TABLE) return -1;
    const set5_pte_t *pte = &pt->entries[index];
...
typedef struct {
    set5_irq_handler_t  handlers[SET5_MAX_IRQS];
    int                 handler_count;
    trit                validity;
} set5_irq_control_t;

static inline void irq_control_init(set5_irq_control_t *ic) {
    if (!ic) return;
    ic->handler_count = 0;
    ic->validity      = TRIT_TRUE;
    for (int i = 0; i < SET5_MAX_IRQS; i++) {
 ...
 */
static inline trit irq_handler_set(set5_irq_control_t *ic, int irq,
                                   int notif_idx, int tcb_id) {
    if (!ic || ic->validity == TRIT_FALSE) return TRIT_FALSE;
    if (irq < 0 || irq >= SET5_MAX_IRQS) return TRIT_FALSE;
 ...
 */
static inline trit irq_handler_ack(set5_irq_control_t *ic, int irq) {
    if (!ic || irq < 0 || irq >= SET5_MAX_IRQS) return TRIT_FALSE;
    set5_irq_handler_t *h = &ic->handlers[irq];
 ...
 *
 * seT5: ASID pools have ternary validity — an Unknown ASID means
 * "address space is being migrated between cores."
 */
#define SET5_ASIDS_PER_POOL  64

typedef struct {
    int   id;
    trit  validity;
    trit  asids[SET5_ASIDS_PER_POOL]; /* T=allocated, U=migrating, F=free */
    int   allocated_count;
} set5_asid_pool_t;

static inline void asid_pool_init(set5_asid_pool_t *pool, int id) {
    if (!pool) return;
    pool->id = id;
    pool->validity = TRIT_TRUE;
    pool->allocated_count = 0;
    for (int i = 0; i < SET5_ASIDS_PER_POOL; i++)
        pool->asids[i] = TRIT_FALSE; /* All free */
}

static inline int asid_pool_alloc(set5_asid_pool_t *pool) {
...
static inline trit asid_pool_free(set5_asid_pool_t *pool, int asid) {
    if (!pool || pool->validity == TRIT_FALSE) return TRIT_FALSE;
    if (asid < 0 || asid >= SET5_ASIDS_PER_POOL) return TRIT_FALSE;
 ...
 *
 * seT5 MDB: each entry has a ternary validity state:
 *   T: active derivation link
 *   U: pending (mid-transfer)
 *   F: revoked (link dead)
 */
#define SET5_MAX_MDB_ENTRIES 512
...
typedef struct {
    set5_mdb_entry_t entries[SET5_MAX_MDB_ENTRIES];
    int              count;
} set5_mdb_t;

static inline void mdb_init(set5_mdb_t *mdb) {
    if (!mdb) return;
    mdb->count = 0;
    for (int i = 0; i < SET5_MAX_MDB_ENTRIES; i++) {
...
static inline int mdb_insert(set5_mdb_t *mdb, int cnode, int slot, int parent) {
    if (!mdb || mdb->count >= SET5_MAX_MDB_ENTRIES) return -1;
    int idx = mdb->count++;
    set5_mdb_entry_t *e = &mdb->entries[idx];
    e->cap_cnode    = cnode;
    e->cap_slot     = slot;
    e->parent       = parent;
    e->first_child  = -1;
    ...
    if (parent >= 0 && parent < SET5_MAX_MDB_ENTRIES) {
        set5_mdb_entry_t *p = &mdb->entries[parent];
        if (p->first_child < 0) {
            p->first_child = idx;
        } else {
            /* Find last sibling */
 ...
 */
static inline void mdb_revoke(set5_mdb_t *mdb, int idx) {
    if (!mdb || idx < 0 || idx >= SET5_MAX_MDB_ENTRIES) return;
    set5_mdb_entry_t *e = &mdb->entries[idx];
...
/**
 * set5_kernel_t — The complete seT5 microkernel state.
 *
...
typedef struct {
    /* Object arrays */
    set5_cnode_t          cnodes[4];              /* CSpace roots (one per domain) */
    set5_tcb_t            tcbs[SET5_MAX_TCBS];
    set5_endpoint_t       endpoints[SET5_MAX_ENDPOINTS];
    set5_notification_t   notifications[SET5_MAX_NOTIFICATIONS];
    set5_untyped_t        untyped[SET5_MAX_UNTYPED];
    set5_frame_t          frames[SET5_MAX_FRAMES];
    set5_page_table_t     page_tables[SET5_MAX_PAGE_TABLES];
    set5_irq_control_t    irq_control;
    set5_asid_pool_t      asid_pools[SET5_MAX_ASID_POOLS];
    set5_mdb_t            mdb;
...
/**
 * @brief Initialise the complete seT5 kernel.
 */
static inline void set5_kernel_init(set5_kernel_t *k) {
    if (!k) return;
    ...
    /* TCBs */
    for (int i = 0; i < SET5_MAX_TCBS; i++)
        tcb_init(&k->tcbs[i], i);
    k->tcb_count = 0;

    /* Endpoints */
    for (int i = 0; i < SET5_MAX_ENDPOINTS; i++)
        endpoint_init(&k->endpoints[i], i);
    k->ep_count = 0;

    /* Notifications */
    for (int i = 0; i < SET5_MAX_NOTIFICATIONS; i++)
        notification_init(&k->notifications[i], i);
    k->ntf_count = 0;

    /* Untyped */
    for (int i = 0; i < SET5_MAX_UNTYPED; i++)
        untyped_init(&k->untyped[i], i, i * 4096, 12);
    k->ut_count = 0;

    /* Frames */
    for (int i = 0; i < SET5_MAX_FRAMES; i++)
        frame_init(&k->frames[i], i, i);
    k->frame_count = 0;

    /* Page tables */
    for (int i = 0; i < SET5_MAX_PAGE_TABLES; i++)
    ...
    /* ASID pools */
    for (int i = 0; i < SET5_MAX_ASID_POOLS; i++)
...
/**
 * seL4-compatible syscall numbers for the seT5 kernel.
 *
 ...
 */
static inline int set5_create_thread(set5_kernel_t *k, trit priority,
                                     trit domain) {
    if (!k || k->tcb_count >= SET5_MAX_TCBS) return -1;
    int idx = k->tcb_count++;
 ...
 */
static inline int set5_create_endpoint(set5_kernel_t *k) {
    if (!k || k->ep_count >= SET5_MAX_ENDPOINTS) return -1;
    int idx = k->ep_count++;
 ...
 */
static inline int set5_create_notification(set5_kernel_t *k) {
    if (!k || k->ntf_count >= SET5_MAX_NOTIFICATIONS) return -1;
    int idx = k->ntf_count++;
 ...
 */
static inline int set5_schedule(set5_kernel_t *k) {
    if (!k) return -1;
 ...
 */
static inline trit set5_ipc_send(set5_kernel_t *k, int ep_idx,
                                 const set5_ipc_msg_t *msg) {
 ...
 */
static inline int set5_untyped_retype(set5_kernel_t *k, int ut_idx,
                                      kobj_type_t type) {
    ...
    int size = 64; /* Default */
    switch (type) {
        case KOBJ_TCB:          size = sizeof(set5_tcb_t); break;
        case KOBJ_ENDPOINT:     size = sizeof(set5_endpoint_t); break;
        case KOBJ_NOTIFICATION: size = sizeof(set5_notification_t); break;
        case KOBJ_FRAME:        size = sizeof(set5_frame_t); break;
        case KOBJ_CNODE:        size = sizeof(set5_cnode_t); break;

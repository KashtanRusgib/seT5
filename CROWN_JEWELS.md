...
All Crown Jewel documentation and code uses ternary-native terminology:
compiled ternary executables are **trinaries** (not "binaries"), ternary
digits are **trits** (not "bits"), and ternary data units are **trytes**
(not "bytes"). "Binary" is used only when referring to base-2 interop,
host substrates, or comparative analysis. See `CONTRIBUTING.md` for the
full policy.

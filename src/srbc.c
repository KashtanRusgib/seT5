...
void srbc_init(srbc_state_t *state)
{
    ...
    state->stability_pct = 100; /* fresh = fully stable */
...
srbc_status_t srbc_tick(srbc_state_t *state)
{
    ...
    int thermal_offset = ((state->temperature_mc - 25000) * SRBC_THERMAL_COEFF_UV) / 1000 / 1000; /* µV/°C → mV */
    ...
    state->stability_pct = (state->ticks > 0)
                               ? (state->stable_ticks * 100) / state->ticks
                               : 100;

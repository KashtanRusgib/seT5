...
{
    if (!s || !name || s->ns_count >= NS_MAX_NAMESPACES)
        return -1;
...
{
    if (!s || s->proc_count >= NS_MAX_PROCESSES)
        return -1;
    if (ns_id < 0 || ns_id >= s->ns_count)
        return -1; /* VULN-62 fix: spawning in the root namespace (ns_id==0) grants
    ...
    if (ns_id == 0 && !(caps & NS_CAP_ROOT_NS))
        return -1;
...
{
    if (!s || proc_idx < 0 || proc_idx >= s->proc_count)
        return NS_ACCESS_DENIED;
    if (target_ns < 0 || target_ns >= s->ns_count)
        return NS_ACCESS_DENIED;
    ...
    {
        if (target_ns != 0)
            s->total_sandboxed++; /* cross-ns by root: audited */
        return NS_ACCESS_GRANTED;
    }

    /* Same namespace: use namespace policy */
    if (p->namespace_id == target_ns)
        return ns->access_policy;

...
Identical to seT5's balanced ternary:
...
| File | Purpose |
|------|---------|
| `include/set5/samsung_tseq.h` | Core types, sequence generation API, inline utilities |
| `include/set5/samsung_tseq_modem.h` | Modem TX/RX interface, symbol mapping, frame types |
| `include/set5/samsung_tseq_mmio.h` | MMIO register map (4 KB at 0x40300000), emulated state |
...
| **seT5 mapping** | Direct (trit = input) | Direct (trit = sequence elem) |
...
## seT5 Integration Notes

1. **Zero translation overhead**: seT5's `trit` type maps directly to the
   patent's ternary alphabet — no encoding/decoding needed.

2. **IPC synergy**: Ternary sequences can be used as seT5 IPC message
...
3. **Multi-radix bridge**: The cyclic-shift codeset structure is compatible
   with seT5's multi-radix framework — base-3 arithmetic maps naturally
   to ternary sequence operations.

4. **Dual-mode reception**: A single seT5 transmitter can serve both

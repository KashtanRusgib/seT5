...
#define PASS()      do { tests_passed++; tests_run++; printf("[PASS]\n"); } while(0)
#define FAIL(msg)   do { tests_failed++; tests_run++; printf("[FAIL] %s\n", msg); } while(0)
#define DIAG(msg)   do { diag_notes++; printf("  [DIAG] %s\n", msg); } while(0)

/* Helper: compile source via bootstrap and return bytecode length (-1 = fail) */
static int try_compile(const char *src, unsigned char *code, int max) {
    return bootstrap_compile(src, code, max);
}
...
static void test_trit_var_declaration(void) {
    ...
    if (len > 0) PASS();
    else FAIL("trit var decl failed to compile");
}
...
static void test_trit_array_declaration(void) {
    ...
    if (len > 0) PASS();
        ...
        FAIL("trit array decl failed to compile");
...
static void test_capability_init_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("capability init function failed to compile");
}
...
static void test_scheduler_priority_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("scheduler priority logic failed to compile");
}
...
static void test_ipc_endpoint_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("IPC endpoint function failed to compile");
}
...
static void test_godel_utility_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("Gödel utility metric failed to compile");
}
...
static void test_godel_axiom_check_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("Gödel axiom check failed to compile");
}
...
static void test_godel_search_fraction_compile(void) {
    ...
    int len = try_compile(
        ...
        code, 512);
    if (len > 0) PASS();
    else FAIL("Gödel BIOPS scheduler failed to compile");
}
...
static void test_ternary_huffman_pattern(void) {
    ...
    if (len > 0) PASS();
        ...
        FAIL("Huffman trit pattern failed");
...
static void test_packed_simd_concept(void) {
    ...
    if (len > 0) {
        ...
        FAIL("trit array loop failed to compile");
...
static void test_multi_function_compile(void) {
    ...
    if (prog && prog->param_count >= 3) {
        ...
        int ok = 1;
        if (strcmp(prog->params[0]->name, "kleene_and") != 0) ok = 0;
        if (strcmp(prog->params[1]->name, "kleene_or") != 0) ok = 0;
        if (strcmp(prog->params[2]->name, "kleene_not") != 0) ok = 0;
        if (!ok) DIAG("Function names didn't match — check parser");
    } else {
...
static void test_verilog_emission_compiles(void) {
    ...
    if (len > 0) PASS();
    else FAIL("bytecode gen for Verilog path failed");
}

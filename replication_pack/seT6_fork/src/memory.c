/**
 * @file memory.c
 * @brief seT6 Physical Memory Manager — Implementation
 *
 ...
 *
 * @see include/set6/memory.h for API documentation
...
void mem_init(set6_mem_t *mem, int num_pages) {
    if (!mem) return;
    if (num_pages > SET6_MAX_PAGES)
        num_pages = SET6_MAX_PAGES;
    if (num_pages < 0)
        num_pages = 0;
    ...
    for (int p = 0; p < SET6_MAX_PAGES; p++) {
        set6_page_t *pg = &mem->pages[p];
        /* All trits → Unknown (the "third" state, not a valid zero) */
        for (int i = 0; i < SET6_PAGE_TRITS; i++)
...
int mem_free(set6_mem_t *mem, int page_idx) {
    ...
    set6_page_t *pg = &mem->pages[page_idx];
...
trit mem_read(const set6_mem_t *mem, int page_idx, int offset) {
    ...
    if (offset < 0 || offset >= SET6_PAGE_TRITS) return TRIT_UNKNOWN;

    const set6_page_t *pg = &mem->pages[page_idx];
...
int mem_write(set6_mem_t *mem, int page_idx, int offset, trit value) {
    if (!mem) return -1;
    if (page_idx < 0 || page_idx >= mem->total_pages) return -1;
    if (offset < 0 || offset >= SET6_PAGE_TRITS) return -1;

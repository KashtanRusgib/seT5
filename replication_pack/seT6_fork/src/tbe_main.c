 ...
 *
 * Minimal userspace shell bootstrapping seT6.  Provides an interactive
 * 15-command environment for I/O, environment-variable management,
 * syscall testing, and balanced-ternary operations.
 *
 * Runs atop the seT6 microkernel (kernel_state_t), leveraging:
...
static void tbe_banner(void) {
    printf(
        "\n"
        "┌─────────────────────────────────────────────┐\n"
        "│  seT6 TBE — Ternary Bootstrap Environment   │\n"
...
int main(void) {
    ...
    while (running) {
        ...
        if (strcmp(cmd, "help") == 0) {
            ...
            printf("TBE: Shutting down seT6 kernel...\n");
            running = 0;

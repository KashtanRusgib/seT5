/*
 * set6.h - seT6 Ternary Microkernel Syscall Interface (TASK-016)
 *
 * Defines the syscall ABI for the seT6 microkernel — a capability-based
...
static inline int t_cap_send(seT6_cap cap, int msg) {
    (void)cap; (void)msg;
    return 0;
}

/* Receive a message on a capability */
static inline int t_cap_recv(seT6_cap cap) {
    (void)cap;
    return 0;
}

/* Grant a capability with restricted rights */
static inline int t_cap_grant(seT6_cap src, seT6_cap *dest, int rights) {
    if (dest) {
...
/* Revoke a capability */
static inline int t_cap_revoke(seT6_cap *cap) {
    if (cap) {
        cap->rights = 0;
        cap->badge = 0;
    }

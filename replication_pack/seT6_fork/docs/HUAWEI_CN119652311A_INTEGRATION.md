# Huawei CN119652311A Ternary Chip — seT6 Integration Guide
...
| Huawei | Voltage | seT6 Balanced | Symbolic      |
|--------|---------|---------------|---------------|
| 0      | VGND    | −1            | TRIT_FALSE    |
...
```c
#include "set6/huawei_cn119652311a.h"
...
## Integration with Existing seT6 Subsystems
...
conversion between seT6's packed format and the Huawei encoding.
...
## Voltage Threshold Specifications (2026 IEDM / TMD Update)

Based on 2026 IEDM process data and TSMC WSe₂ TMD characterization,
the following threshold specifications apply to the Huawei ternary ALU:

### Multi-Threshold Voltage (MTV) Design

| Threshold | Voltage Range | Application                    |
|-----------|--------------|--------------------------------|
| **LVT**   | 0.20 – 0.30V | Fast-switching logic paths      |
| **MVT**   | 0.40V         | Standard arithmetic (+1/−1 ops) |
| **HVT**   | 0.60 – 0.70V | Low-leakage storage, SRAM cells |

### Design Rules

1. **Stability Sum Constraint**: `V_th(LVT) + V_th(MVT) < V_DD`
   - At 2nm: V_DD ≈ 0.8V → 0.3V + 0.4V = 0.7V < 0.8V ✓
   - Ensures valid ternary noise margins across all three logic levels

2. **Adder/Multiplier Operations**:
   - Use MVT transistors (0.4V) for +1/−1 carry-free arithmetic
   - NTI/PTI gate primitives operate at MVT for balanced speed/power
   - Compensation units (+6, +9) use LVT for critical-path timing

3. **Static Noise Margin (SNM)**:
   - SNM ≥ 50mV at nominal conditions (typical corner, 25°C)
   - SNM ≥ 30mV at worst-case (slow corner, 125°C, LVT minimum)
   - Three-level ternary requires tighter margins than binary

4. **TMD Drift Budget** (per TSMC WSe₂ endurance data):
   - Threshold drift: ~1mV per decade of cycles
   - At 10¹² cycle endurance: total drift ~12mV << SNM of 50mV
   - Design margin: 3× safety factor over expected drift

5. **PVT Corner Matrix**:
   - 3 process corners × 3 voltage levels × 3 temperatures = 27 configs
   - All configurations tested by `tstab_pvt_full_sweep()` in test suite
   - Worst-case SNM at (Slow, LVT, 125°C) must exceed 30mV

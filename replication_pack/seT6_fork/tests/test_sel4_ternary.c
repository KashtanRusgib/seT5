 ...
 * @brief Test suite for the seT6 seL4 Ternary Kernel Object Model
 *
...
int main(void) {
    printf("seT6 seL4 Ternary Kernel — Test Suite\n");
    printf("======================================\n\n");
    ...
    set6_cap_t root_cap = cap_mk(KOBJ_CNODE, 0, TRIT_TRUE, TRIT_TRUE, TRIT_TRUE);
    ...
    set6_cap_t child_cap = cap_derive(&root_cap, TRIT_TRUE, TRIT_FALSE, TRIT_FALSE, 42);
    ...
    /* Unknown derivation */
    set6_cap_t pending_cap = cap_derive(&root_cap, TRIT_UNKNOWN, TRIT_UNKNOWN, TRIT_FALSE, 99);
    ...
    /* Null cap */
    set6_cap_t null_cap = cap_mk_null();
    ...
    set6_cnode_t *cn = &k.cnodes[0];
    CHECK("cnode: valid", cn->validity == TRIT_TRUE);
    CHECK("cnode: slot_count = 0", cn->slot_count == 0);

    /* Insert caps */
    set6_cap_t cap1 = cap_mk(KOBJ_ENDPOINT, 0, TRIT_TRUE, TRIT_TRUE, TRIT_FALSE);
    ...
    /* Lookup */
    set6_cap_t *looked = cnode_lookup(cn, 0);
    ...
    int t0 = set6_create_thread(&k, TRIT_TRUE, TRIT_TRUE);
    int t1 = set6_create_thread(&k, TRIT_UNKNOWN, TRIT_UNKNOWN);
    int t2 = set6_create_thread(&k, TRIT_FALSE, TRIT_FALSE);
    ...
    int sched_result = set6_schedule(&k);
    ...
    /* Message transfer */
    set6_ipc_msg_t msg = {{TRIT_TRUE, TRIT_FALSE, TRIT_UNKNOWN, TRIT_TRUE}, 42, t0};
    ...
    /* IPC send via kernel */
    CHECK("ipc: kernel send ok", set6_ipc_send(&k, ep, &msg) == TRIT_TRUE);

    /* ==== Notifications ================================================ */
    printf("\n-- Notifications --\n");

    int ntf = set6_create_notification(&k);
    ...
    k.ut_count = 1;
    set6_untyped_t *ut = &k.untyped[0];
    CHECK("ut: initial watermark = 0", ut->watermark == 0);

    /* Retype into TCB */
    int child_idx = set6_untyped_retype(&k, 0, KOBJ_TCB);
    ...
    child_idx = set6_untyped_retype(&k, 0, KOBJ_ENDPOINT);
    ...
    set6_page_table_t *pt = &k.page_tables[0];
    ...
    set6_asid_pool_t *pool = &k.asid_pools[0];

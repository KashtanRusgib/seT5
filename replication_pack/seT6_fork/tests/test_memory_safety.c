 ...
 * @brief Memory Safety and Bounds Checking Tests for seT6
 *
 * Tests memory allocation, deallocation, bounds checking,
 * and scrub-on-free guarantees using the actual seT6 memory API.
 */
...
void test_memory_read_write(void) {
    ...
    wr = mem_write(&mem, page, SET6_PAGE_TRITS - 1, TRIT_FALSE);
    CHECK("Write at max offset", wr == 0);

    /* Read at max offset */
    val = mem_read(&mem, page, SET6_PAGE_TRITS - 1);
    CHECK("Read at max offset", val == TRIT_FALSE);

    /* Out-of-bounds write should fail */
    wr = mem_write(&mem, page, SET6_PAGE_TRITS, TRIT_TRUE);
    CHECK("Out-of-bounds write rejected", wr == -1);

    /* Out-of-bounds read returns Unknown */
    val = mem_read(&mem, page, SET6_PAGE_TRITS);
...
int main(void) {
    printf("=== seT6 Memory Safety Tests ===\n\n");

...
enum Opcode
{
    ...
    OP_DIV, /* 36: Pop b, a; push a/b (0 if b==0) */
    OP_MOD, /* 37: Pop b, a; push a%b (0 if b==0) */

    /* === Phase 4: SIMD Packed64 operations === */
    OP_PACK64,   /* 38: Pack 32 trits from stack into one 64-bit word, push result */
    OP_UNPACK64, /* 39: Pop packed64, push 32 individual trit values */
    OP_SIMD_AND, /* 40: Pop b, a (packed64); push packed AND (trit_min per slot) */
    OP_SIMD_OR,  /* 41: Pop b, a (packed64); push packed OR (trit_max per slot) */
    OP_SIMD_NEG, /* 42: Pop a (packed64); push packed NEG (flip all trits) */

...
void pf_free(PostfixSeq *seq)
{
    if (seq->instrs)
    {
        ...
        {
            if (seq->instrs[i].name)
                free(seq->instrs[i].name);
        }
        free(seq->instrs);
    }
    seq->instrs = NULL;
...
static void emit_ast(PostfixSeq *seq, Expr *e)
{
    ...
    case NODE_BINOP:
        ...
        case OP_IR_ADD:
            ...
            break;
        case OP_IR_SUB:
            pf_emit(seq, PF_SUB, 0, NULL);
            break;
        case OP_IR_MUL:
            pf_emit(seq, PF_MUL, 0, NULL);
            break;
        case OP_IR_DIV:
            pf_emit(seq, PF_DIV, 0, NULL);
            break;
        case OP_IR_MOD:
            pf_emit(seq, PF_MOD, 0, NULL);
            break;
        case OP_IR_CMP_EQ:
            pf_emit(seq, PF_CMP_EQ, 0, NULL);
            break;
        case OP_IR_CMP_LT:
            pf_emit(seq, PF_CMP_LT, 0, NULL);
            break;
        case OP_IR_CMP_GT:
            pf_emit(seq, PF_CMP_GT, 0, NULL);
            break;
        case OP_IR_NEG:
            pf_emit(seq, PF_NEG, 0, NULL);
            break;
        }
        break;
    ...
    while (changed)
    {
        ...
        if (changed)
        {
            ...
            for (int read = 0; read < seq->count; read++)
            {
                ...
                {
                    if (seq->instrs[read].name)
                        free(seq->instrs[read].name);
                }
            }
            seq->count = write;
        }

(* =========================================================================
   ...
   of the seT5 microkernel using balanced ternary capabilities.
...
theorem authority_confinement:
  ...
  by (cases "cap_read p"; cases "cap_read c";
      cases "cap_write p"; cases "cap_write c";
      cases "cap_grant p"; cases "cap_grant c";
      cases "cap_valid p"; cases "cap_valid c") auto
...
lemma sec_label_bottom: "\<forall>x::trit. F \<le> x" by (cases x) auto
lemma sec_label_top: "\<forall>x::trit. x \<le> T" by (cases x) auto
...
theorem cia_guarantee:
  "\<forall>c::capability.
   (* Confidentiality: revoked caps lose read *)
   (cap_valid c = F \<longrightarrow> operation_allowed c = F) \<and>
   (* Integrity: derivation is monotonic *)
   (cap_read (cap_derive c c) \<le> cap_read c) \<and>
   (* Availability: valid caps always granted *)
   (cap_valid c = T \<longrightarrow> operation_allowed c = T)"

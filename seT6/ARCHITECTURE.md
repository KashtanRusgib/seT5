...
# seT6 Architecture — Secure Embedded Ternary Microkernel 6
...
seT6 is a **ground-up rewrite of the seL4 verified microkernel** in balanced
...
which is indistinguishable from a valid zero.  In seT6, uninitialized data
...
seT6's ISA is designed to run on emerging ternary hardware:

| Vendor/Tech     | Key Development                                   | seT6 Alignment                          |
|-----------------|---------------------------------------------------|-----------------------------------------|
...
| **RISC-V**      | Open ISA predicted as 2026 AI "dark horse" ($260B by 2030) | seT6 ISA extension model inspired by RISC-V custom instructions |
...
symmetry).  seT6 targets these same efficiency gains.
...
seT6 uses a **postfix (reverse Polish) instruction encoding**, matching the
...
seT6's two-stack model descends directly from the **DSSP (Dialogue System
...
Forth adopted.  seT6 restores this design to its native ternary substrate.
...
seT6 provides **16 general-purpose trit registers** (R0–R15), each holding
...
seT6 inherits seL4's capability model, extended with ternary access rights.
...
Defined in `tools/compiler/include/set6.h`.
...
The full syscall ABI is defined in `tools/compiler/include/set6.h`.
...
seT6 includes **instruction-level hooks** for mixed-radix computation,
...
The **TBE** (`src/tbe_main.c`) is seT6's minimal userspace shell — the
...
│  └────────────┘  └────────────┘  └────────────────────┘    │
├─────────────────────────────────────────────────────────────┤
│                kernel_state_t (seT6 microkernel)            │
...
├──────────────────────────────────────────────────────────────┤
│  Trit Linux Compatibility  (include/set6/posix.h)           │
├──────────────────────────────────────────────────────────────┤
│  TBE Bootstrap Shell  (src/tbe_main.c — 15 commands)        │
├──────────────────────────────────────────────────────────────┤
│  seL4 Kernel Object Model  (include/set6/sel4_ternary.h)    │
...
├──────────────────────────────────────────────────────────────┤
│  Syscall ABI     (include/set6/syscall.h — 14 syscalls)     │
├──────────────────────────────────────────────────────────────┤
│  Multi-Radix     (include/set6/multiradix.h)                │
│    DOT_TRIT · FFT_STEP · RADIX_CONV · load balance          │
├──────────────────────────────────────────────────────────────┤
│  WCET / Telemetry (include/set6/wcet.h, qemu_trit.h)       │
├──────────────────────────────────────────────────────────────┤
│  Device Driver    (include/set6/dev_trit.h — /dev/trit)     │
├──────────────────────────────────────────────────────────────┤
...
Abstract Model (HOL)  ──refines──►  Executable Spec (C)  ──validates──►  Binary
  ...
  (future: CapSafety.thy)     set6.h (syscall ABI)             Verilog netlist
...
A Linux compatibility layer running atop the seT6 microkernel:
...
- **Huawei CFET (2026-2030):** Balanced ternary AI chip per patent
  ...
  seT6 ISA is directly compatible with patent's gate-level design.
...
- Brusentsov & Zhogolev (1970, 1979) — DSSP/Setun-70 two-stack architecture;
  seT6's operand+return stack model is a direct descendant
